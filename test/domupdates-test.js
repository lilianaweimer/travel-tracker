...
describe('domUpdates', function() {
  ...
  it('should spy on prettifyMoneyNumbers', function () {
    let dollars = 123;

    global.domUpdates;
    chai.spy.on(domUpdates, ['prettifyMoneyNumbers'], () => {});
    domUpdates.prettifyMoneyNumbers(dollars);
    expect(domUpdates.prettifyMoneyNumbers).to.have.been.called(1);
    expect(domUpdates.prettifyMoneyNumbers).to.have.been.called.with(dollars);
  });

  it('should spy on prettifyTripDate', function () {
    let date = {};

    global.domUpdates;
    chai.spy.on(domUpdates, ['prettifyTripDate'], () => {});
    domUpdates.prettifyTripDate(date);
    expect(domUpdates.prettifyTripDate).to.have.been.called(1);
    expect(domUpdates.prettifyTripDate).to.have.been.called.with(date);
  });
});
